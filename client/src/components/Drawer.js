...
export default function Drawer({ navigation, label, align, handleIsLogin }) {
  ...
  const buttons = [
    { index: 0, type: 'button', label: 'Recargar dinero', icon: 'arrow-collapse-up', route: 'Recharge' },
    { index: 1, type: 'button', label: 'Enviar dinero', icon: 'subdirectory-arrow-right', route: 'Transfer' },
    { index: 2, type: 'button', label: 'Transacciones', icon: 'refresh', route: 'Transaction' },
    { index: 3, type: 'separator' },
    { index: 4, type: 'button', label: 'Mis tarjetas', icon: 'credit-card', route: 'Card' },
    { index: 5, type: 'button', label: 'Soporte', icon: 'help-circle', route: 'Support' },
  ...
  const IconButtonText = ({ labelButton, icon, route }) => (
    <TouchableOpacity
      onPress={
        route ? () => navigation.navigate(route) : () => handleLogOut()
      }
        ...
        <View style={styles.profile}>
          ...
          <View style={styles.container}>
            <View style={styles.avatar}>
              <Text text={user && user.name && user.name[0].toUpperCase()} type='subtitle1' style={{color: 'white'}}/>
            </View>
            <View style={{flexDirection: 'row', flex: 1}}>
              <View style={styles.text}>
                <Text text={`Hola ${user && user.name}`} type='body2' color='light'/>
